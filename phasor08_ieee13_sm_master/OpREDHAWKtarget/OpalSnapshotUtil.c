...
int OpalSnapshot_Copy_DWork(void * src, void * dst) {
   ...
   eltSize = sizeof(pSrc->DelayToavoidalgebraicloop_DSTAT);
   memcpy(&pDst->DelayToavoidalgebraicloop_DSTAT, &pSrc->DelayToavoidalgebraicloop_DSTAT, eltSize);
   size += eltSize;
   ...
   eltSize = sizeof(pSrc->ToHold_PreviousInput);
   memcpy(&pDst->ToHold_PreviousInput, &pSrc->ToHold_PreviousInput, eltSize);
   size += eltSize;
